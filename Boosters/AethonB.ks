...
local box_APalt is wndw:addhlayout().
	...
	local APaltvalue is box_APalt:ADDTEXTFIELD("400").
...
local box_LVL_Sp is wndw:addhlayout().
	...
	local LVL_Spvalue is box_LVL_Sp:ADDTEXTFIELD("5000").
...
local box_Circ_Sp is wndw:addhlayout().
	local Circ_Sp_label is box_Circ_Sp:addlabel("Booster Circ Speed").
...
until (TIME:SECONDS - EngineStartTime) > 40{
	Wait 0.2.
}
Stage.//remove solid boosters
until staging{
